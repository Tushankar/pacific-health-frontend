...
import { Send, User, Search, Phone, MoreHorizontal, ArrowLeft, MessageSquare, CheckCheck, Loader2, Edit2, Trash2, X, MoreVertical } from "lucide-react";
    ...
    if (token) {
      ...
      socket.on("message_edited", (data) => {
        setRealtimeMessages((prev) =>
          prev.map((m) => (m._id === data._id ? { ...m, message: data.message, isEdited: true } : m))
        );
        queryClient.setQueryData(["chatHistory", selectedContact?._id], (old = []) =>
          old.map((m) => (m._id === data._id ? { ...m, message: data.message, isEdited: true } : m))
        );
      });

      socket.on("message_deleted_everyone", (data) => {
        setRealtimeMessages((prev) =>
          prev.map((m) => (m._id === data._id ? { ...m, deletedForEveryone: true } : m))
        );
        queryClient.setQueryData(["chatHistory", selectedContact?._id], (old = []) =>
          old.map((m) => (m._id === data._id ? { ...m, deletedForEveryone: true } : m))
        );
      });

      socket.on("message_deleted_me", (data) => {
        setRealtimeMessages((prev) =>
          prev.map((m) => (m._id === data.messageId ? { ...m, isDeletedForMe: true } : m))
        );
        queryClient.setQueryData(["chatHistory", selectedContact?._id], (old = []) =>
          old.map((m) => (m._id === data.messageId ? { ...m, isDeletedForMe: true } : m))
        );
      });

      socket.on("messages_read", (data) => {
        setRealtimeMessages((prev) =>
          prev.map((m) => (m.sender?._id === data.readerId || m.sender === data.readerId ? m : { ...m, isRead: true }))
        );
        queryClient.setQueryData(["chatHistory", selectedContact?._id], (old = []) =>
          old.map((m) => (m.sender?._id === data.readerId || m.sender === data.readerId ? m : { ...m, isRead: true }))
        );
      });
      ...
      markAsRead(selectedContact._id)
        .then(() => {
          ...
          if (socketRef.current) {
            socketRef.current.emit("mark_read", { senderId: selectedContact._id });
          }
        })
        .catch(console.error);
    }
  ...
  return (
    <div className="h-[calc(100vh-87px)] bg-[#F4F7F9] p-4 md:p-6 lg:p-8 font-inter overflow-hidden flex flex-col">
      <div className="max-w-[1400px] w-full mx-auto h-full flex gap-6">
        ...
        <div className={`w-full md:w-[350px] lg:w-[400px] bg-white rounded-[24px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200/60 flex flex-col overflow-hidden transition-all duration-300 ${selectedContact ? 'hidden md:flex' : 'flex'}`}>
          {/* Header & Search */}
          <div className="p-6 border-b border-slate-100 flex-shrink-0 bg-white z-10">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h2 className="text-[22px] font-black text-slate-800 tracking-tight leading-none">Messages</h2>
                <p className="text-[12px] font-bold text-slate-400 mt-1 uppercase tracking-widest">Team Communication</p>
              </div>
            </div>

            <div className="relative group">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                <Search size={16} strokeWidth={2.5} className="text-slate-400 group-focus-within:text-blue-500 transition-colors" />
              </div>
              <input
                type="text"
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3 space-y-1 custom-scrollbar">
            {loadingContacts ? (
              <div className="flex flex-col items-center justify-center h-full gap-3 text-slate-400">
                ...
                <span className="text-sm font-semibold tracking-wide">Syncing chats...</span>
              </div>
            ) : filteredContacts.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-slate-400">
                <Search size={32} className="mb-3 opacity-30 text-slate-500" strokeWidth={2} />
                <span className="text-sm font-semibold tracking-wide">No conversations found</span>
              </div>
            ) : (
              filteredContacts.map((contact) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3.5 px-4 py-3.5 rounded-2xl transition-all duration-300 text-left group border ${isSelected
                  ...
                  >
                    <div className="relative flex-shrink-0">
                      <div className={`w-[46px] h-[46px] rounded-full overflow-hidden transition-all duration-300 shadow-sm ${isSelected ? 'ring-2 ring-blue-500 ring-offset-2' : 'group-hover:ring-2 group-hover:ring-slate-200 group-hover:ring-offset-1'}`}>
                        <img src={avatarUrl} alt={contact.fullName} className="w-full h-full object-cover" />
                      </div>
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                      <div className="flex justify-between items-center mb-1">
                        <h3 className={`font-bold truncate text-[14px] tracking-tight transition-colors ${isSelected ? "text-blue-900" : "text-slate-800"}`}>
                          {contact.fullName}
                        </h3>
                        {contact.lastMessage && (
                          <span className={`text-[10px] whitespace-nowrap ml-2 font-bold uppercase tracking-widest ${contact.unreadCount > 0 ? "text-blue-600" : "text-slate-400"}`}>
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <p className={`text-[12.5px] truncate font-semibold leading-snug tracking-wide ${isSelected ? "text-blue-700/80" : contact.unreadCount > 0 ? "text-slate-800 font-bold" : "text-slate-500"}`}>
                          {contact.lastMessage ? contact.lastMessage.content : (contact.role === 'admin' ? 'Administrator' : 'Contact')}
                        </p>
        ...
        <div className={`flex-1 bg-white rounded-[24px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200/60 overflow-hidden flex flex-col transition-all duration-300 relative ${!selectedContact ? 'hidden md:flex' : 'flex'}`}>
          {selectedContact ? (
            <>
              {/* Chat Header */}
              <div className="px-6 py-5 flex items-center justify-between border-b border-slate-100 bg-white z-20 relative shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-3.5">
                    ...
                    <div>
                      <h3 className="font-black text-slate-800 tracking-tight leading-tight text-[16px]">{selectedContact.fullName}</h3>
                      <div className="flex items-center gap-1.5 opacity-90 mt-0.5">
                        ...
                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-none">{selectedContact.role === 'admin' ? 'Administrator' : 'Online'}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-5 bg-slate-50/30 custom-scrollbar relative">
                ...
                <div className="absolute inset-0 opacity-[0.015] pointer-events-none" style={{ backgroundImage: 'radial-gradient(circle at 2px 2px, #0f172a 1px, transparent 0)', backgroundSize: '16px 16px' }}></div>
                ...
                {loadingHistory ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-center max-w-sm mx-auto">
                    <div className="w-16 h-16 bg-blue-50 rounded-2xl flex items-center justify-center text-blue-500 mb-5 shadow-[0_4px_20px_rgba(37,99,235,0.08)] transform -rotate-6">
                      <MessageSquare size={28} strokeWidth={2} className="transform rotate-6" />
                    </div>
                    <h3 className="text-[18px] font-black text-slate-800 tracking-tight mb-2">Say Hello 👋</h3>
                    <p className="text-slate-500 text-[13px] font-medium leading-relaxed">It's quiet in here. Send a message to start the conversation with {selectedContact.fullName.split(' ')[0]}.</p>
                  </div>
                ) : (
                  allMessages.map((msg, index) => {
                    const isMe = msg.sender._id === user._id || msg.sender === user._id;
                    const isLastInGroup = index === allMessages.length - 1 ||
                      (allMessages[index + 1] && (allMessages[index + 1].sender._id || allMessages[index + 1].sender) !== (msg.sender._id || msg.sender));
                    ...
                    return (
                      ...
                      >
                        <div className={`flex items-center gap-2 max-w-[80%] lg:max-w-[65%] group ${isMe ? "flex-row-reverse" : "flex-row"}`}>
                          <div
                          ...
                          >
                            {(msg.deletedForEveryone || msg.isDeletedForMe || msg.deletedForMe?.includes(user._id)) ? (
                              <p className={`text-[14px] font-medium leading-[1.6] tracking-wide break-words whitespace-pre-wrap italic opacity-80 flex items-center gap-2 ${isMe ? 'text-blue-200' : 'text-slate-500'}`}>
                                <Trash2 size={14} className="opacity-70" /> {isMe ? "You deleted this message" : "This message was deleted"}
                              </p>
                            ) : (
                              <p className="text-[14px] font-medium leading-[1.6] tracking-wide break-words whitespace-pre-wrap">{msg.message}</p>
                            )}

                            <div className={`flex items-center gap-1.5 mt-1.5 ${isMe ? "justify-end text-blue-200" : "justify-start text-slate-400"}`}>
                              ...
                              {msg.isEdited && !(msg.deletedForEveryone || msg.isDeletedForMe || msg.deletedForMe?.includes(user._id)) && <span className="text-[9.5px] font-bold tracking-widest uppercase italic">(edited)</span>}
                              {isMe && <CheckCheck size={14} strokeWidth={2.5} className={`leading-none transition-colors ${msg.isRead ? "text-red-500" : "opacity-50 text-slate-400"}`} />}
                            </div>
                          </div>
                          ...
                          {(!msg.deletedForEveryone && !msg.isDeletedForMe && !msg.deletedForMe?.includes(user._id)) && (
                            <div className="relative flex-shrink-0">
                              <button
                                onClick={() => setActiveMessageOptions(activeMessageOptions === msg._id ? null : msg._id)}
                              ...
                              {activeMessageOptions === msg._id && (
                                <>
                                  ...
                                  ></div>
                                  <div className={`absolute ${index === allMessages.length - 1 ? "bottom-full mb-1" : "top-full mt-1"} ${isMe ? "right-0" : "left-0"} bg-white rounded-xl shadow-[0_4px_24px_rgba(0,0,0,0.12)] border border-slate-100/80 py-1.5 min-w-[180px] z-[50]`}>
                                    {isMe && (
                                      <>
                                        <button onClick={() => startEdit(msg)} className="w-full flex items-center gap-2.5 px-3.5 py-2 hover:bg-slate-50 text-[13px] font-semibold text-slate-700 transition-colors">
                                          <Edit2 size={14} className="text-blue-500" /> Edit Message
                                        </button>
                                        <button onClick={() => deleteForEveryone(msg._id)} className="w-full flex items-center gap-2.5 px-3.5 py-2 hover:bg-slate-50 text-[13px] font-semibold text-red-600 transition-colors">
                                          <Trash2 size={14} /> Delete for everyone
                                        </button>
                                      </>
                                    )}
                                    <button onClick={() => deleteForMe(msg._id)} className="w-full flex items-center gap-2.5 px-3.5 py-2 hover:bg-slate-50 text-[13px] font-semibold text-slate-700 transition-colors relative z-[51]">
                                      <X size={14} className="text-slate-500" /> Delete for me
                                    </button>
                                  </div>
                                </>
                              )}
              ...
              <div className="p-4 md:p-5 bg-white border-t border-slate-100/80 z-20 shadow-[0_-4px_24px_rgba(0,0,0,0.02)]">
                {editingMessageId && (
                  <div className="max-w-4xl mx-auto mb-3 px-4 py-2.5 bg-blue-50/50 rounded-[12px] border border-blue-100/50 flex flex-row items-center justify-between text-[13px] text-blue-800 font-semibold shadow-sm">
                    ...
                    <button onClick={cancelEdit} className="p-1 hover:bg-blue-100 rounded-full text-blue-600 transition-colors">
                      <X size={15} />
                    </button>
                  </div>
                )}
                <div className="flex items-end gap-3 max-w-4xl mx-auto bg-slate-50 p-1.5 rounded-[20px] border border-slate-200/50 shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)] focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-400/50 focus-within:bg-white transition-all cursor-text"
                  onClick={(e) => {
                  ...
                  <div className="flex items-center gap-2 pb-1 pr-1">
                    ...
                    >
                      <Send size={18} strokeWidth={2.5} className={`${message.trim() ? "group-hover:translate-x-0.5 group-hover:-translate-y-0.5" : ""} transition-transform duration-300`} />
                    </button>
                  </div>
                </div>
                <div className="text-center mt-3 hidden lg:block">
                  <span className="text-[9.5px] uppercase font-bold tracking-widest text-slate-300">Press <span className="text-slate-400">ENTER</span> to send, <span className="text-slate-400">SHIFT+ENTER</span> for new line</span>
                </div>
              </div>
            </>
          ) : (
            ...
            <div className="flex-1 flex items-center justify-center bg-slate-50/50 relative">
              ...
              <div className="text-center max-w-sm px-6 animate-in fade-in duration-700 slide-in-from-bottom-4">
                ...
                <p className="text-slate-500 text-[14px] font-medium leading-relaxed">
                  Select a conversation from the sidebar to chat, or start a new conversation to connect with your contacts securely.
                </p>
              </div>
            </div>
          )}
