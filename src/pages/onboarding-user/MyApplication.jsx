...
const MyApplication = () => {
  ...
  const { data: enrollmentData, isLoading } = useQuery({
    ...
    queryFn: () =>
      enrollmentId ? getEnrollmentById(enrollmentId) : getMyEnrollment(),
  ...
  const progressStats = useMemo(() => {
    ...
    const approvedCount = visibleForms.filter(
      (f) => f.status === "approved",
    ).length;
    const rejectedCount = visibleForms.filter(
      (f) => f.status === "rejected",
    ).length;
    const completedStatus = visibleForms.filter(
    ...
    const completed = isApproved
      ? total
      : approvedCount + rejectedCount + completedStatus;
      ...
      <div className="space-y-8 animate-in fade-in duration-700">
          ...
          <div className="relative z-10 w-full">
            ...
            {(activeEnrollment?.status === "approved" ||
              ...
              activeEnrollment?.adminNote && (
                <div
                  className={`mt-2 mb-4 p-4 rounded-2xl border flex gap-3 items-start animate-in slide-in-from-top-2 duration-500 ${activeEnrollment?.status === "approved" ? "bg-emerald-50 border-emerald-100 text-emerald-800" : "bg-rose-50 border-rose-100 text-rose-800"}`}
                >
                  <MessageSquare
                    size={18}
                    className={
            ...
            <p className="text-slate-500 max-w-3xl text-sm md:text-base">
              Complete your documentation to finalize your admission. Your
              progress is saved automatically as you fill out each form. All
              required documents must be submitted, reviewed, and approved by
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[
            ...
            {
              label: "Remaining",
              value: progressStats.actionRequired,
              icon: AlertCircle,
            ...
            >
              <div
                className={`p-3 rounded-xl ${stat.bg} ${stat.color} shadow-sm border border-current/10`}
              >
                <stat.icon size={20} />
              </div>
              <div>
        ...
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-3 sm:p-4 md:p-6 border-b border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto">
              ...
              <span className="flex items-center gap-1 sm:gap-1.5 text-[10px] sm:text-xs text-slate-400 font-bold uppercase tracking-tighter">
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50/50">
                  <th className="px-3 sm:px-4 md:px-6 py-2.5 sm:py-3 md:py-4 text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                    Status
                  </th>
                  <th className="px-3 sm:px-4 md:px-6 py-2.5 sm:py-3 md:py-4 text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                    Document Details
                  </th>
                  <th className="px-3 sm:px-4 md:px-6 py-2.5 sm:py-3 md:py-4 text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest hidden sm:table-cell">
                    Admin Feedback / Notes
                  </th>
                  <th className="px-3 sm:px-4 md:px-6 py-2.5 sm:py-3 md:py-4 text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest hidden md:table-cell">
                    Method
                  </th>
                  <th className="px-3 sm:px-4 md:px-6 py-2.5 sm:py-3 md:py-4 text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest text-right">
                    Action
                  </th>
                </tr>
              </thead>
                ...
                {dashboardFormsData.map((chapter) => (
                  <React.Fragment key={chapter.chapter}>
                    <tr className="bg-slate-50/70 border-y border-slate-200/50">
                      ...
                      >
                        <div className="flex items-center gap-2 sm:gap-3">
                          <div className="h-5 sm:h-6 md:h-7 w-1 sm:w-1.5 bg-gradient-to-b from-blue-600 to-blue-900 rounded-full shadow-sm"></div>
                          <span className="text-sm sm:text-base font-black bg-gradient-to-r from-blue-700 to-blue-900 bg-clip-text text-transparent uppercase tracking-tight">
                            {chapter.chapter}
                          </span>
                        </div>
                      </td>
                    ...
                    {chapter.forms.map((form) => {
                      const backendForm = activeEnrollment?.forms?.find(
                        (f) => f.formId === form.id,
                      );
                      const isCompleted = backendForm?.status === "completed";
                      const isInProgress =
                        backendForm?.status === "in-progress";
                      ...
                      if (backendForm?.status === "approved")
                        status = "Approved";
                      else if (backendForm?.status === "rejected")
                        status = "Rejected";
                      ...
                      const note =
                        status === "Rejected" && backendForm?.adminNote
                          ? backendForm.adminNote
                          : status === "Approved" && backendForm?.adminNote
                            ? backendForm.adminNote
                            : status === "Approved"
                              ? "Document approved."
                              : isCompleted
                      ...
                      return (
                        ...
                        >
                          <td className="px-3 sm:px-4 md:px-6 py-3 sm:py-4 md:py-5">
                            <span
                              className={`px-1.5 sm:px-2 md:px-2.5 py-0.5 sm:py-1 rounded-full text-[9px] sm:text-[10px] font-black uppercase border tracking-wider whitespace-nowrap ${statusColors[status]}`}
                            >
                              {status}
                            </span>
                          </td>
                          <td className="px-3 sm:px-4 md:px-6 py-3 sm:py-4 md:py-5">
                            <div className="text-xs sm:text-sm font-bold text-slate-800 group-hover:text-blue-600 transition-colors">
                              {form.name}
                            </div>
                            <div className="text-[10px] sm:text-[11px] text-slate-400 font-medium whitespace-nowrap">
                              #{form.id} • {chapter.chapter.split("-")[0]}
                            </div>
                          </td>
                          ...
                          <td className="px-3 sm:px-4 md:px-6 py-3 sm:py-4 md:py-5 hidden md:table-cell">
                            <span className="px-1.5 sm:px-2 py-0.5 sm:py-1 bg-slate-100 text-slate-600 rounded-lg text-[9px] sm:text-[10px] font-black uppercase whitespace-nowrap">
                              {form.type}
                            </span>
                          </td>
        ...
        onKeyDownCapture={(e) => {
          if (readOnly) {
            ...
            if (
              ...
              e.stopPropagation();
              toast.info(
                "This form is submitted to admin. You cannot change it unless any instruction from admin arrive.",
                {
  ...
  const renderFormComponent = () => {
    ...
    if (Component) {
      ...
      if (activeEnrollment) {
        ...
        phase1Completed = phase1Forms.filter((f) =>
          ["completed", "approved", "rejected"].includes(f.status),
        ).length;
      }
      ...
      if (formsData) {
        ...
        const currentIndex = allVisibleForms.findIndex(
        ...
        <div className="bg-white rounded-3xl shadow-2xl border border-slate-200">
          ...
          <div className="bg-gradient-to-r from-slate-50 to-white border-b border-slate-200 p-3 sm:p-4 md:p-6 rounded-t-3xl">
            <div className="flex items-center gap-2 sm:gap-3 md:gap-4 mb-3 md:mb-4">
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                <FileText size={20} className="text-slate-600 sm:w-6 sm:h-6" />
                <div>
                  <h1 className="text-base sm:text-xl md:text-2xl font-bold text-slate-800">
                    {selectedForm.name}
                  </h1>
                  <p className="text-xs sm:text-sm text-slate-600">
                    {selectedForm.chapter}
                  </p>
                </div>
              </div>
            </div>
            ...
            {(() => {
              ...
              if (
                ...
                return (
                  <div className="mx-3 sm:mx-6 md:mx-8 mt-3 sm:mt-4 p-3 sm:p-4 rounded-xl sm:rounded-2xl border flex gap-2 sm:gap-3 md:gap-4 items-start bg-rose-50 border-rose-100">
                    <div className="p-1.5 sm:p-2 rounded-lg sm:rounded-xl bg-rose-100 text-rose-600 flex-shrink-0">
                      <MessageSquare size={14} className="sm:w-4 sm:h-4" />
                    </div>
                    <div>
                      <h4 className="font-bold mb-1 text-rose-900 text-xs sm:text-sm">
                        Review Feedback
                      </h4>
                      <p className="text-[11px] sm:text-xs leading-relaxed text-rose-700 font-medium">
                        {backendForm?.adminNote}
                      </p>
                    </div>
                  </div>
              ...
              if (
                backendForm?.status === "approved" &&
                backendForm?.adminNote
              ) {
                return (
                  <div className="mx-3 sm:mx-6 md:mx-8 mt-3 sm:mt-4 p-3 sm:p-4 rounded-xl sm:rounded-2xl border flex gap-2 sm:gap-3 md:gap-4 items-start bg-emerald-50 border-emerald-100">
                    <div className="p-1.5 sm:p-2 rounded-lg sm:rounded-xl bg-emerald-100 text-emerald-600 flex-shrink-0">
                      <MessageSquare size={14} className="sm:w-4 sm:h-4" />
                    </div>
                    <div>
                      <h4 className="font-bold mb-1 text-emerald-900 text-xs sm:text-sm">
                        Review Comments
                      </h4>
                      <p className="text-[11px] sm:text-xs leading-relaxed text-emerald-700 font-medium">
                        {backendForm?.adminNote}
                      </p>
                    </div>
                  </div>
            ...
            {(activeEnrollment?.status === "rejected" ||
              ...
              activeEnrollment?.adminNote && (
                <div
                  className={`mx-3 sm:mx-6 md:mx-8 mt-4 sm:mt-6 md:mt-8 p-4 sm:p-5 md:p-6 rounded-2xl sm:rounded-3xl border flex gap-2 sm:gap-3 md:gap-4 items-start ${activeEnrollment.status === "approved" ? "bg-emerald-50 border-emerald-100" : "bg-rose-50 border-rose-100"}`}
                >
                  <div
                    className={`p-2 sm:p-2.5 md:p-3 rounded-xl sm:rounded-2xl flex-shrink-0 ${activeEnrollment.status === "approved" ? "bg-emerald-100 text-emerald-600" : "bg-rose-100 text-rose-600"}`}
                  >
                    <MessageSquare size={16} className="sm:w-5 sm:h-5" />
                  </div>
                  <div>
                    <h4
                      className={`font-bold mb-1 text-sm sm:text-base ${activeEnrollment.status === "approved" ? "text-emerald-900" : "text-rose-900"}`}
                    >
                      Admissions Feedback
                    </h4>
                    <p
                      className={`text-xs sm:text-sm leading-relaxed font-medium ${activeEnrollment?.status === "approved" ? "text-emerald-700" : "text-rose-700"}`}
                    >
                      {activeEnrollment?.adminNote}
                    </p>
                  </div>
