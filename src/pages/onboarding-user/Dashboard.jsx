...
import { getMyEnrollment, createEnrollment, updateFormStatus, submitEnrollment } from "../../api/enrollment.api";
...
const ClientManagementHub = () => {
  ...
  const updateFormMutation = useMutation({
    mutationFn: ({ enrollmentId, formId, status }) => updateFormStatus(enrollmentId, formId, status),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["myEnrollment"] });
    },
  });
      ...
      return [
        {
          ...
          forms: [
            { id: 1, name: "Client Information Form", type: "Data Entry", desc: "Patient demography and background" },
            { id: 2, name: "Service Agreement Form", type: "Fillable", desc: "Service agreement form" },
            { id: 3, name: "Service Agreement Addendum", type: "Fillable", desc: "Addendum to service agreement" },
            { id: 4, name: "Service Care Plan", type: "Fillable", desc: "Care plan details" },
            { id: 5, name: "Initial Comprehensive Assessment", type: "Fillable", desc: "Initial assessment" },
            { id: 6, name: "Risk Mitigation Plan", type: "Fillable", desc: "Risk mitigation details" },
            { id: 7, name: "Self-Preservation", type: "Fillable", desc: "Self-preservation assessment" },
            { id: 8, name: "Medication List", type: "Fillable", desc: "List of medications" },
            { id: 9, name: "Doctor's Orders", type: "Upload", desc: "Upload doctor's orders" },
            { id: 10, name: "Abnormal Involuntary Movement Scale (AIMS)", type: "Fillable", desc: "AIMS assessment" },
          ],
        },
        {
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
        <div className="bg-white rounded-[32px] w-full max-w-lg overflow-hidden shadow-2xl animate-in slide-in-from-bottom-8 duration-500 border border-slate-200">
          ...
          <div className="p-8 text-center">
            <h3 className="text-2xl font-bold text-slate-800 mb-2">Confirm Selection</h3>
            <p className="text-slate-500 mb-6 text-sm">
              You are about to enroll in the <span className="font-bold text-indigo-600">{selectedProgram}</span> program.
              <br /><br />
              <span className="p-3 bg-amber-50 text-amber-800 rounded-xl border border-amber-200 inline-block font-bold">
                Important: Once confirmed, you cannot change your program until you complete all forms and receive admin approval.
              </span>
            </p>
            ...
            <div className="flex gap-4">
              ...
              >
                {createMutation.isPending ? <Loader2 className="animate-spin" size={20} /> : <CheckCircle size={20} />}
                Confirm & Start
              </button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-12 gap-10">
            ...
            <div className="col-span-12 lg:col-span-9 space-y-12">
              {formsData.map((chapter, idx) => (
                <section
                  ...
                  onClick={() => {
                    if (chapter.isLocked) {
                      toast.info("Locked: This section will unlock after you complete submitting all forms properly and receive admin approval.");
                    }
                  }}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 relative">
                    {chapter.isLocked && (
                      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-white/40 backdrop-blur-[2px] rounded-3xl border-2 border-dashed border-slate-200/50">
                        ...
                        <p className="font-bold text-slate-800 text-lg">Coming in Phase 2</p>
                        <p className="text-slate-500 text-sm max-w-xs text-center px-4">These documents will be available once your initial packet is approved by our admin team.</p>
                      </div>
                    )}
                    {chapter.forms
                      .filter((f) =>
                      ...
                      .map((form) => (
                        <div
                          key={form.id}
                          className={`bg-white rounded-2xl border p-6 hover:shadow-xl hover:shadow-indigo-500/5 transition-all group flex flex-col justify-between ${
                            form.status === 'completed' ? 'border-indigo-200 bg-indigo-50/10' : 
                            form.status === 'approved' ? 'border-emerald-200 bg-emerald-50/10' :
                            form.status === 'rejected' ? 'border-rose-200 bg-rose-50/10' :
                        ...
                        >
                          <div>
                            <div className="flex justify-between items-start mb-4">
                              ...
                              <div className="flex flex-col items-end">
                                ...
                                {form.status && (
                                  <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded-md mt-1 ${
                                    form.status === 'completed' ? 'bg-indigo-100 text-indigo-600' :
                                    form.status === 'approved' ? 'bg-emerald-100 text-emerald-600' :
                                    form.status === 'rejected' ? 'bg-rose-100 text-rose-600' :
                                    form.status === 'in-progress' ? 'bg-amber-100 text-amber-600' :
                            ...
                            <h3 className="font-bold text-slate-800 leading-tight mb-2 group-hover:text-indigo-600 transition-colors">
                              {form.name}
                            </h3>
                            
                            {/* Admin Note Display inside Card */}
                            {(form.status === 'rejected' || (form.status === 'approved' && form.adminNote)) && form.adminNote && (
                              <div className={`mt-2 p-2 rounded-lg text-[10px] leading-snug border ${
                                form.status === 'rejected' ? 'bg-rose-50 border-rose-100 text-rose-700' : 'bg-emerald-50 border-emerald-100 text-emerald-700'
                              }`}>
                                <div className="flex items-center gap-1 mb-1 font-bold">
                                  <MessageSquare size={10} />
                                  <span>{form.status === 'rejected' ? 'Feedback:' : 'Note:'}</span>
                                </div>
                                "{form.adminNote}"
                              </div>
                            )}
                          ...
                          <button
                            onClick={() => navigate(`/my-application?formId=${form.id}`)}
                            className={`w-full relative overflow-hidden flex items-center justify-between p-3 rounded font-bold shadow-md hover:shadow-lg transition-all mt-4 group/btn ${
                              form.status === 'completed' ? 'bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 shadow-indigo-200 text-white' : 
                              form.status === 'approved' ? 'bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 shadow-emerald-200 text-white' :
                              form.status === 'rejected' ? 'bg-gradient-to-r from-rose-500 to-red-600 hover:from-rose-600 hover:to-red-700 shadow-rose-200 text-white' :
                          ...
                          >
                            ...
                            <span className="text-xs px-1 relative z-20">
                              {form.status === 'completed' || form.status === 'approved' ? 'Review Document' : 
                               form.status === 'rejected' ? 'Fix Issues' :
                               form.status === 'in-progress' ? 'Continue Workflow' : 'Open Workflow'}
                            </span>
            ...
            <div className="col-span-12 lg:col-span-3">
              <div className="sticky top-32 space-y-6">
                  ...
                  <div className="bg-slate-900 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden group">
                    ...
                    {(() => {
                      ...
                      if (activeEnrollment) {
                        ...
                        completedForms = phase1Forms.filter(f => ["completed", "approved", "rejected"].includes(f.status)).length;
                      }

                      const percentage = activeEnrollment.status === "approved" ? 100 : totalForms > 0 ? Math.round((completedForms / totalForms) * 100) : 0;
                      
                      return (
                        <>
                          <div className="flex items-end gap-2 mb-6 relative z-10">
                            <span className="text-5xl font-bold italic">{percentage}%</span>
                            <span className="text-indigo-300 text-sm font-bold mb-1">
                              Phase 1 Completion
                            </span>
                          </div>
                          ...
                          {activeEnrollment.status === 'submitted' || (activeEnrollment.status === 'pending' && activeEnrollment.submittedAt) ? (
                            <div className="w-full py-4 bg-amber-500/20 border border-amber-500/30 rounded-2xl flex flex-col items-center gap-2 text-amber-200">
                              <Clock size={24} />
                              <span className="text-xs font-bold uppercase tracking-wider text-center">Awaiting Admin Review</span>
                            </div>
                          ) : activeEnrollment.status === 'rejected' ? (
                            <button
                             onClick={() => submitMutation.mutate(activeEnrollment._id)}
                             disabled={submitMutation.isPending || percentage < 100}
                             className="w-full bg-rose-600 py-4 rounded-2xl font-bold text-sm shadow-lg hover:bg-rose-500 transition-all flex items-center justify-center gap-2"
                            >
                             {submitMutation.isPending ? <Loader2 className="animate-spin" size={20} /> : <AlertTriangle size={20} />}
                             Resubmit Application
                            </button>
                          ) : (
                            <button
                              onClick={() => submitMutation.mutate(activeEnrollment._id)}
                              disabled={submitMutation.isPending || percentage < 100 || activeEnrollment.status === 'approved'}
                              className={`w-full py-4 rounded-2xl font-bold text-sm shadow-lg transition-all flex items-center justify-center gap-2 ${
                                percentage === 100 ? 'bg-indigo-600 hover:bg-indigo-500 text-white' : 'bg-slate-800 text-slate-500 cursor-not-allowed'
                              }`}
                            >
                              {submitMutation.isPending ? <Loader2 className="animate-spin" size={20} /> : <ClipboardList size={20} />}
                              {activeEnrollment.status === 'approved' ? 'Application Approved' : 'Submit Full Packet'} <ChevronRight size={18} />
                            </button>
                          )}
                          
                          {percentage < 100 && !activeEnrollment.submittedAt && (
                            <p className="text-[10px] text-slate-400 mt-4 text-center">
                               Please complete all {totalForms - completedForms} remaining documents to enable submission.
                            </p>
                          )}
                        </>
                      );
                ...
                {activeEnrollment?.status === 'rejected' && activeEnrollment?.adminNote && (
                  <div className="bg-rose-50 border border-rose-200 rounded-3xl p-6 shadow-sm">
                    <div className="flex items-center gap-2 mb-3 text-rose-600">
                      <XCircle size={18} />
                      <h4 className="text-sm font-bold">Admin Feedback</h4>
                    </div>
                    <p className="text-xs text-rose-800 leading-relaxed italic border-l-2 border-rose-300 pl-3">
                ...
                <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm">
                  <h4 className="text-xs font-black text-blue-900 uppercase tracking-widest mb-6 border-b border-slate-50 pb-4">
                    Onboarding Legend
                  </h4>
                  <div className="space-y-4">
                    {[
                      ...
                      { type: "Signable", color: "bg-amber-500", icon: PenTool },
                      ...
                      >
                        <div
                          className={`w-8 h-8 rounded-lg ${item.color}/10 flex items-center justify-center text-slate-600`}
                        >
                          <item.icon
                            size={16}
                ...
                <div className="p-6 rounded-3xl bg-amber-50 border border-amber-100">
                  ...
                  <p className="text-[12px] text-amber-700 leading-relaxed font-medium">
                    Documents in **Chapter III** for COMP and **Chapter II** for
                    NOW are unlocked automatically after the Initial Admission
                    Packet is validated by Admin.
                  </p>
                </div>
              </div>
            </div>
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-start pt-16 p-6 font-sans antialiased text-slate-900">
        ...
        <div className="bg-white p-8 rounded-3xl shadow-xl border border-slate-200">
          {activeEnrollment && (
            <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-amber-800">Enrollment Active</p>
                <p className="text-xs text-amber-600">Your selection is locked until admin review.</p>
              </div>
            </div>
          )}
          ...
          <div className="space-y-3 mb-8">
            {programOptions.map((p) => (
              ...
              >
                <div>
                  <p className={`font-bold ${selectedProgram === p.value ? "text-indigo-900" : "text-slate-700"}`}>
                    {p.label}
                  </p>
                  <p className="text-xs text-slate-500 mt-1">{p.description}</p>
                </div>
                <ChevronRight size={18} className={selectedProgram === p.value ? "text-indigo-600" : "text-slate-300"} />
              </button>
            ))}
          </div>
