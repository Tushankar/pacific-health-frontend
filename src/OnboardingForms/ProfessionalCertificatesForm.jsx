...
const ProfessionalCertificatesForm = ({
  enrollmentId,
  formId,
  onComplete,
  savedData,
  ...
  isReadOnly = false,
  onNext,
  activeEnrollment, // Added to detect position from other forms
}) => {
  ...
  useEffect(() => {
    // Detect position from Job Description form in activeEnrollment
    const jobDescForm = activeEnrollment?.forms?.find(f => f.name === "Job Description" || f.formId === 102);
    const selectedPos = jobDescForm?.data?.positionSelected || jobDescForm?.data?.positionType || localStorage.getItem("onboarding_position") || "PCA";
    setPositionType(selectedPos);
  }, [activeEnrollment]);

  const getCertificateType = (pos) => {
    const certMap = {
      PCA: "PCA Certificate",
      CNA: "CNA Certificate",
      LPN: "LPN License",
    ...
    return certMap[pos] || "Professional Certificate";
  };
    ...
    <div className="flex flex-col-reverse 2xl:flex-row w-full items-start bg-white text-black font-sans">
      ...
      <div className="flex-1 flex flex-col items-center mt-4 mb-8 w-full">
        <div className="w-[98%] md:w-[85%] lg:w-[75%] bg-white shadow-xl rounded-2xl border border-gray-200 overflow-hidden">
          ...
          {/* Status Banner - EXACTLY LIKE HRMS */}
          <div className={`p-4 border-b ${existingFile ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}>
            <div className="flex items-center justify-center gap-3">
              ...
              <div>
                {existingFile ? (
                  <>
                    <p className="text-base font-semibold text-green-800">✅ 1 document(s) uploaded successfully</p>
                    <p className="text-sm text-green-600 mt-1">Review your document below or replace it if needed.</p>
                  </>
                ) : (
                  <p className="text-base font-semibold text-red-800">⚠️ Not filled yet - Upload your documents to complete your progress</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="p-8">
            {/* Title Section - EXACTLY LIKE HRMS */}
            <div className="text-center mb-8">
              <div className="mb-4">
                <h2 className="text-2xl font-bold text-blue-600 mb-2">Professional Certificate</h2>
                <p className="text-gray-700 text-lg">
                  You have selected: <span className="font-semibold text-blue-800">{getCertificateType(positionType)}</span>
                </p>
              </div>
              <h1 className="text-3xl font-bold text-gray-800 mb-2">{getCertificateType(positionType)} Document Upload</h1>
              <p className="text-gray-600 font-medium italic">Upload your {getCertificateType(positionType).toLowerCase()}</p>
            </div>

            {/* Instructions Section - EXACTLY LIKE HRMS */}
                ...
                <div className="flex-1">
                  ...
                  <ol className="space-y-3 text-sm text-gray-700">
                    <li className="flex gap-3">
                      ...
                      <span>Prepare your {getCertificateType(positionType).toLowerCase()} (PDF or image format)</span>
                    </li>
                    <li className="flex gap-3">
                      <span className="font-bold text-blue-600 flex-shrink-0">2.</span>
                      <span>Click "Choose File" or drag and drop to upload your certificate</span>
                    </li>
                    <li className="flex gap-3">
                      <span className="font-bold text-blue-600 flex-shrink-0">3.</span>
                      <span>Once uploaded, you can view or remove the certificate if needed</span>
                    </li>
                    <li className="flex gap-3">
            ...
            {/* Step 1 Section - EXACTLY LIKE HRMS */}
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Step 1: Upload Document</h2>
              <p className="text-gray-600 text-sm mb-6 italic">Supported formats: PDF, JPG, PNG (Max 10MB)</p>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <h3 className="text-sm font-medium text-green-800 truncate">{existingFile.originalName || "certificate.pdf"}</h3>
...
ProfessionalCertificatesForm.propTypes = {
...
export default ProfessionalCertificatesForm;
