...
const ClientServicePlan = ({ onComplete, savedData, progressCurrent = 0, progressTotal = 1, onFormChange, isReadOnly = false, onNext }) => {
      ...
      <div className="flex-1 flex flex-col items-center mt-4 mb-8">
        ...
        <div className="border border-black mb-1">
          <div className="grid grid-cols-1 md:grid-cols-[3fr_1fr] border-b border-black text-[9px] md:text-sm">
            <div className={`p-1 border-b md:border-b-0 md:border-r border-black flex ${errors.name ? "bg-red-50" : ""}`}>
              <span className="font-bold mr-1">Name: <RequiredStar /> </span>
              <input
                name="name"
                value={formData.name}
                onChange={(e) => {
            ...
            <div className={`p-1 flex ${errors.telephone ? "bg-red-50" : ""}`}>
              <span className="font-bold mr-1">Telephone: <RequiredStar /> </span>
              <input
                name="telephone"
          ...
          <div className="grid grid-cols-1 md:grid-cols-[3fr_2fr_1fr] border-b border-black">
            <div className={`p-1 border-b md:border-b-0 md:border-r border-black flex ${errors.address ? "bg-red-50" : ""}`}>
              <span className="font-bold mr-1">Address: <RequiredStar /> </span>
              <input
                name="address"
            ...
            <div className={`p-1 border-b md:border-b-0 md:border-r border-black flex ${errors.city ? "bg-red-50" : ""}`}>
              <span className="font-bold mr-1">City: <RequiredStar /> </span>
              <input
                name="city"
                value={formData.city}
                onChange={(e) => {
            ...
            <div className={`p-1 flex ${errors.age ? "bg-red-50" : ""}`}>
              <span className="font-bold mr-1">Age: <RequiredStar /> </span>
              <input
                name="age"
                value={formData.age}
                onChange={(e) => {
          ...
          <div className={`p-1 ${errors.diagnoses ? "bg-red-50" : ""}`}>
            <span className="font-bold block">Client Diagnoses: <RequiredStar /> </span>
            <textarea
              name="diagnoses"
              value={formData.diagnoses}
              onChange={(e) => {
                handleChange(e);
                if(errors.diagnoses) setErrors(prev => ({...prev, diagnoses: null}));
              }}
        ...
        <div className="border border-black border-t-0 p-1 mb-0 border-b border-black bg-gray-50">
          <span className="font-bold mr-1 block md:inline">Functional Limitations:</span>
          <div className="inline-flex gap-2 flex-wrap">
            {[
                {label: "Unsteady gait", name: "limitations.unsteadyGait"},
                {label: "HOH", name: "limitations.hoh"},
                {label: "Impaired Speech", name: "limitations.impairedSpeech"},
                {label: "R/L sided weakness", name: "limitations.rlWeakness"},
                {label: "Decreased Sensory", name: "limitations.decreasedSensory"},
                ...
                <label key={lim.name} className="flex items-center gap-1">
                    <input
                        type="checkbox"
                        name={lim.name}
                        checked={lim.name.split('.').reduce((o, i) => o[i], formData)}
                        onChange={handleChange}
                    />
                    {lim.label}
                </label>
          ...
          <div className="border-b md:border-b-0 md:border-r border-black">
            ...
            <div className="flex flex-col">
              {[
                ...
                <label key={name} className="flex items-center gap-2 border-b border-black p-1">
                  <input
                    type="checkbox"
                    name={name}
              ...
              <label className="flex items-center gap-2 border-b border-black p-1">
               ...
               {[
                ...
                { label: "Medication Reminder", name: "tasks.medicationReminder" },
                { label: "Pick up prescriptions", name: "tasks.prescriptions" },
          ...
          <div className="flex flex-col">
            ...
            <div className="flex flex-col">
               {[
                ...
                { label: "Accompany to MD appointment", name: "extraTasks.mdAppointment" },
                { label: "Provide watchful oversight", name: "extraTasks.oversight" },
                { label: "Clean Bathroom", name: "extraTasks.cleanBathroom" },
                { label: "Incontinence Care", name: "extraTasks.incontinenceCare" },
                { label: "Vital Signs", name: "extraTasks.vitalSigns" },
                { label: "Blood Sugar", name: "extraTasks.bloodSugar" },
              ].map(({ label, name }) => (
                <label key={name} className="flex items-center gap-2 border-b border-black p-1 last:border-b-0 md:last:border-b">
                  <input
                    type="checkbox"
                    name={name}
                    checked={name.split('.').reduce((o, i) => o[i], formData)}
        ...
        <div className="border border-black border-t-0 mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 border-b border-black">
                <div className={`p-1 border-b md:border-b-0 md:border-r border-black flex items-center ${errors["initialPlan.date"] ? "bg-red-50" : ""}`}>
                    <span className="font-bold mr-2">Initial Plan Date: <RequiredStar /> </span>
                    <input
                        type="date"
                        ...
                        onChange={(e) => {
                          handleChange(e);
                          if(errors["initialPlan.date"]) setErrors(prev => ({...prev, "initialPlan.date": null}));
                        }}
                        style={getStyle("initialPlan.date")}
                        className={`flex-1 ${errors["initialPlan.date"] ? "border-red-500" : ""}`}
                    />
                </div>
                <div className={`p-1 flex items-center ${errors["initialPlan.sig1"] ? "bg-red-50" : ""}`}>
                    <span className="font-bold mr-2">Signature: <RequiredStar /> </span>
                    <input
                        name="initialPlan.sig1"
                        value={formData.initialPlan.sig1}
                        onChange={(e) => {
                          handleChange(e);
                          if(errors["initialPlan.sig1"]) setErrors(prev => ({...prev, "initialPlan.sig1": null}));
                        }}
        ...
        {/* Submit Button */}
          ...
          {/* Action Buttons */}
          <div className="w-full px-2 md:px-8 py-8 flex justify-between items-center">
            <button
              type="button"
