...
const DrivingLicenseForm = ({
  ...
  useEffect(() => {
    // Detect governmentIdType from Personal Information form in activeEnrollment
    const personalInfoForm = activeEnrollment?.forms?.find(f => f.name === "Personal Information" || f.formId === 101);
    const idType = personalInfoForm?.data?.governmentIdType || "";
    setGovernmentIdType(idType.toLowerCase());
  }, [activeEnrollment]);
  ...
  const getFormTitle = () => {
    if (governmentIdType === "driver's license") return "Driver's License Upload";
    if (governmentIdType === "passport") return "Passport Upload";
    if (governmentIdType === "state id") return "State ID Upload";
        ...
        <div className="w-[98%] md:w-[85%] lg:w-[75%] bg-white shadow-xl rounded-2xl border border-gray-200 overflow-hidden">
          ...
          <div className={`p-4 border-b ${existingFile ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}>
            <div className="flex items-center justify-center gap-3">
              ...
              <div>
                {existingFile ? (
                  <>
                    <p className="text-base font-semibold text-green-800">✅ Progress Updated - Document Uploaded Successfully</p>
                    <p className="text-sm text-green-600 mt-1">Review your document below or replace it if needed.</p>
                  </>
                ) : (
                  <p className="text-base font-semibold text-red-800">⚠️ Not filled yet - Upload your document(s) to complete your progress</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="p-8">
            ...
            <div className="text-center mb-8">
              <h1 className="text-3xl font-bold text-gray-800 mb-2">{getFormTitle()}</h1>
              <p className="text-gray-600 font-medium">Upload your Driver's License or any other means of photo identification</p>
            </div>
            ...
            {governmentIdType && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
                <div className="flex items-center gap-2 text-green-700">
                  ...
                  <span className="font-medium">Selected ID Type: {governmentIdType.charAt(0).toUpperCase() + governmentIdType.slice(1)}</span>
                </div>
              </div>
            )}

            {/* Instructions Section - MATCHING HRMS */}
                ...
                <div className="flex-1">
                  ...
                  <ol className="space-y-3 text-sm text-gray-700">
                    ...
                    <li className="flex gap-3">
                      ...
                      <span>Ensure the document is clear and all information is readable</span>
                    </li>
                    <li className="flex gap-3">
                      <span className="font-bold text-blue-600 flex-shrink-0">3.</span>
                      <span>Save the document as a PDF, JPG, or PNG file</span>
                    </li>
                    <li className="flex gap-3">
                    ...
                    <li className="flex gap-3">
                      ...
                      <span>Upload the ID whose information was provided in the applicant information section</span>
                    </li>
                  </ol>
                </div>
              </div>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
              <h2 className="text-xl font-semibold text-gray-800 mb-2">Upload Government ID Document(s)</h2>
              ...
              {existingFile && !file ? (
                <div className="space-y-6">
                  <div>
                    ...
                    <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-3 shadow-sm">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          <File className="w-6 h-6 text-green-600 flex-shrink-0" />
                          <div className="flex-1 min-w-0">
                            <h3 className="text-sm font-medium text-green-800 truncate">{existingFile.originalName || "government_id.pdf"}</h3>
                ...
                <div className="space-y-4">
                  <div 
                    className={`border-2 border-dashed rounded-lg p-10 text-center transition-all cursor-pointer ${
                      isReadOnly ? "opacity-50 border-gray-200" : "border-gray-300 bg-white hover:border-blue-500 hover:bg-blue-50"
                    }`}
            ...
            {/* Document Info Section - MATCHING HRMS */}
            <div className="mt-8 p-6 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-100">
              <div className="flex items-center gap-2 mb-4">
                <Target className="w-5 h-5 text-blue-600" />
                <span className="text-base font-semibold text-gray-700">📋 Document Information</span>
              </div>
              <ul className="space-y-3 text-sm text-gray-700">
                ...
                <li className="flex gap-3">
                  <span className="font-bold text-blue-600 flex-shrink-0">•</span>
                  <span>{governmentIdType ? `${governmentIdType.charAt(0).toUpperCase() + governmentIdType.slice(1)} must be valid (not expired)` : "Government-issued Picture ID must be valid (not expired)"}</span>
                </li>
                <li className="flex gap-3">
                  <span className="font-bold text-blue-600 flex-shrink-0">•</span>
                  <span>{governmentIdType === "passport" ? "Information page should be clearly visible" : "Both front and back should be clearly visible"}</span>
                </li>
                <li className="flex gap-3">
                  <span className="font-bold text-blue-600 flex-shrink-0">•</span>
                  <span>{governmentIdType ? `Selected ID type: ${governmentIdType.charAt(0).toUpperCase() + governmentIdType.slice(1)}` : "Accepted IDs: Driver's License, Passport, State ID, etc."}</span>
                </li>
              </ul>
            </div>
